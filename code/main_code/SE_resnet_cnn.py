...
# Model configuration
model_name = 'se_resnet50'   # Options: 'se_resnet18', 'se_resnet34', 'se_resnet50', 'se_resnet101', 'se_resnet152'
...
# Ensure 'timm' library is installed for SE-ResNet models
try:
    import timm
except ImportError:
    raise ImportError("The 'timm' library is required for SE-ResNet models. Install it using 'pip install timm'.")
...
def load_se_resnet(model_name, num_classes):
    """
    Load a SE-ResNet model from the 'timm' library.

    Args:
        model_name (str): Name of the SE-ResNet model.
        num_classes (int): Number of output classes.

    Returns:
        model (nn.Module): Modified SE-ResNet model.
    """
    model = timm.create_model(model_name, pretrained=False, num_classes=num_classes, global_pool='avg')
    return model

# Available SE-ResNet models in 'timm'
available_se_resnets = [
    'se_resnet18',
    'se_resnet34',
    'se_resnet50',
    'se_resnet101',
...
if model_name not in available_se_resnets:
    raise ValueError(f"Invalid model name. Choose from {available_se_resnets}.")

model = load_se_resnet(model_name, num_attributes)
...
def save_predictions(predictions, labels, iteration):
    ...
    filename = f'se_resnet_predictions_{model_name}_iteration{iteration}_{timestamp}.csv'
    ...
    report_filename = f'se_resnet_classification_report_{model_name}_iteration{iteration}_{timestamp}.csv'
...
def plot_training_curves(train_losses, val_losses, train_jaccards, val_jaccards, iteration):
    ...
    plt.ylabel('Loss')
    plt.legend()
    loss_plot_filename = f'se_resnet_loss_plot_{model_name}_iteration{iteration}_{timestamp}.png'
    ...
    plt.legend()
    acc_plot_filename = f'se_resnet_accuracy_plot_{model_name}_iteration{iteration}_{timestamp}.png'
