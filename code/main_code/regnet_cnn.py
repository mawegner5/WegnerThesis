...
# Model configuration
model_name = 'regnet_y_400mf'   # Options: 'regnet_y_400mf', 'regnet_y_800mf', 'regnet_y_1_6gf', etc.
...
# Input size for RegNet models
input_size = 224  # RegNet models use 224x224 images
...
if model_name == 'regnet_y_400mf':
    ...
    model = regnet_y_1_6gf(weights=None)
elif model_name == 'regnet_y_3_2gf':
    model = regnet_y_3_2gf(weights=None)
elif model_name == 'regnet_y_8gf':
    model = regnet_y_8gf(weights=None)
elif model_name == 'regnet_y_16gf':
    model = regnet_y_16gf(weights=None)
elif model_name == 'regnet_y_32gf':
    model = regnet_y_32gf(weights=None)
elif model_name == 'regnet_y_128gf':
    model = regnet_y_128gf(weights=None)
else:
    raise ValueError("Invalid model name. Choose from 'regnet_y_400mf', 'regnet_y_800mf', etc.")
...
def save_predictions(predictions, labels, iteration):
    ...
    filename = f'regnet_predictions_{model_name}_iteration{iteration}_{timestamp}.csv'
    ...
    report_filename = f'regnet_classification_report_{model_name}_iteration{iteration}_{timestamp}.csv'
...
def plot_training_curves(train_losses, val_losses, train_jaccards, val_jaccards, iteration):
    ...
    plt.ylabel('Loss')
    plt.legend()
    loss_plot_filename = f'regnet_loss_plot_{model_name}_iteration{iteration}_{timestamp}.png'
    ...
    plt.legend()
    acc_plot_filename = f'regnet_accuracy_plot_{model_name}_iteration{iteration}_{timestamp}.png'
