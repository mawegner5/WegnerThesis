...
model_name = 'vit_b_16'  # 'vit_b_16', 'vit_b_32', 'vit_l_16', 'vit_l_32'

num_epochs_initial = 150
early_stopping_patience_initial = 25
batch_size = 32
learning_rate = 1e-5  # Lowered to help avoid NaNs
weight_decay = 1e-5
optimizer_name = 'Adam'
threshold = 0.5
dropout_rate = 0.5
...
# Vision Transformer input sizes
input_sizes = {
    'vit_b_16': 224,
    'vit_b_32': 224,
    'vit_l_16': 224,
...
def train_initial_model():
    ...
    data_transforms = {
        'train': transforms.Compose([
            ...
            # Removed ColorJitter to reduce potential instability
            transforms.RandomRotation(5),  # Lower rotation
            transforms.ToTensor(),
    ...
    # Final validation pass to save predictions
    model.eval()
    final_preds = []
    final_labels = []
    final_img_names = []
    ...
    with torch.no_grad():
        for inputs, labels, img_names in dataloaders['validate']:
            ...
            final_preds.append(preds_binary.cpu().numpy())
            final_labels.append(labels.cpu().numpy())
            final_img_names.extend(img_names)

    if len(final_preds) > 0 and len(final_labels) > 0:
        final_preds = np.vstack(final_preds)
        final_labels = np.vstack(final_labels)
        save_predictions(final_preds, final_labels, final_img_names, 'initial', model_name)
    else:
    ...
    save_performance_summary(model_name, best_jaccard, final_val_loss, time_elapsed, 'initial', {
...
def optuna_objective(trial):
    ...
    learning_rate_ = trial.suggest_float('learning_rate', 1e-6, 1e-4, log=True)  # narrower
    ...
    final_val_preds = np.vstack(final_val_preds) if len(final_val_preds) > 0 else []
    final_val_labels = np.vstack(final_val_labels) if len(final_val_labels) > 0 else []
